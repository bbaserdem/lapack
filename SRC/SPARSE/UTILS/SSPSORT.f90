...
CONTAINS
    ...
    SUBROUTINE DSPSORT_COO(COO, INFO)
        ...
        SEALLOCATE(perm, temp_row, temp_col, temp_val)
    ...
    RECURSIVE SUBROUTINE quicksort_coo_indices(row_ind, col_ind, perm, first, last)
        ...
        IF (first < last) THEN
            ...
            SO
                ...
                SO WHILE (row_ind(perm(i)) < pivot_row .OR. &
                         (row_ind(perm(i)) == pivot_row .AND. col_ind(perm(i)) < pivot_col))
                    i = i + 1
                END SO
                
                SO WHILE (row_ind(perm(j)) > pivot_row .OR. &
    ...
    RECURSIVE SUBROUTINE quicksort_with_values(indices, values, first, last)
        ...
        IF (first < last) THEN
            ...
            SO
                SO WHILE (indices(i) < pivot)
                    i = i + 1
                END SO
                SO WHILE (indices(j) > pivot)
                    j = j - 1
                END SO
                IF (i >= j) EXIT
