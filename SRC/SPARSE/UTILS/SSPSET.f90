!> \brief \b SSPSET sets an element value in a sparse matrix
!>
!> \par Purpose:
!> =============
!>
!> SSPSET sets the value of element (I,J) in a sparse matrix.
...
!>          Note: Currently only COO format is supported for SSPSET.
!>
!> \param[in] I
!>          I is INTEGER
...
SUBROUTINE DSPSET_COO(COO, I, J, VALUE, INFO)
    ...
    IF (found) THEN
        IF (ABS(VALUE) < SPARSE_ZERO_TOL) THEN
    ...
    ELSE
        ...
        IF (ABS(VALUE) >= SPARSE_ZERO_TOL) THEN
            ...
            IF (COO%nnz >= COO%nnz_alloc) THEN
                ...
                SEALLOCATE(COO%row_ind, COO%col_ind, COO%values)
