...
!> that may create duplicates (like SSPADD2) or zeros (like cancellation).
!>
...
SUBROUTINE SSPCOMP(FORMAT, SPARSE, TOL, INFO)
    ...
    INTERFACE
        SUBROUTINE SSPSORT(FORMAT, SPARSE, INFO)
            CHARACTER, INTENT(IN) :: FORMAT
            CLASS(*), INTENT(INOUT) :: SPARSE
            INTEGER, INTENT(OUT) :: INFO
        END SUBROUTINE SSPSORT
    END INTERFACE
    
    ! Initialize
    INFO = 0
    tolerance = 0.0_real32
    IF (PRESENT(TOL)) tolerance = ABS(TOL)
...
CONTAINS
    ...
    SUBROUTINE DSPCOMP_CSR(CSR, TOL, INFO)
        ...
        REAL(real32), ALLOCATABLE :: new_values(:)
        ...
        SO i = 1, CSR%nrows
            ...
            SO WHILE (j <= row_end)
                ...
                k = j + 1
                SO WHILE (k <= row_end .AND. CSR%col_ind(k) == current_col)
        ...
        SEALLOCATE(new_col_ind, new_values, new_row_ptr)
    ...
    SUBROUTINE DSPCOMP_CSC(CSC, TOL, INFO)
        ...
        SO j = 1, CSC%ncols
            ...
            SO WHILE (i <= col_end)
                ...
                k = i + 1
                SO WHILE (k <= col_end .AND. CSC%row_ind(k) == current_row)
        ...
        SEALLOCATE(new_row_ind, new_values, new_col_ptr)
        
    END SUBROUTINE DSPCOMP_CSC

END SUBROUTINE SSPCOMP