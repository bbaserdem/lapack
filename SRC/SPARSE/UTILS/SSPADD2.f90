...
SUBROUTINE SSPADD2(FORMAT, M, N, ALPHA, A, BETA, B, C, INFO)
...
CONTAINS
    ...
    SUBROUTINE DSPADD2_COO(M, N, ALPHA, A, BETA, B, C, INFO)
        INTEGER(int32), INTENT(IN) :: M, N
        REAL(real32), INTENT(IN) :: ALPHA, BETA
        TYPE(sparse_coo_s), INTENT(IN) :: A, B
        TYPE(sparse_coo_s), INTENT(OUT) :: C
        INTEGER(int32), INTENT(OUT) :: INFO
        ...
        ! Special cases
        IF (ALPHA == 0.0_real32 .AND. BETA == 0.0_real32) THEN
            ! C = 0
            C%nrows = M
            C%ncols = N
            C%nnz = 0
        ...
        ! Call SSPCOMP afterwards to combine duplicates
    ...
    SUBROUTINE DSPADD2_CSR(M, N, ALPHA, A, BETA, B, C, INFO)
        INTEGER(int32), INTENT(IN) :: M, N
        REAL(real32), INTENT(IN) :: ALPHA, BETA
        TYPE(sparse_csr_s), INTENT(IN) :: A, B
        TYPE(sparse_csr_s), INTENT(OUT) :: C
        INTEGER(int32), INTENT(OUT) :: INFO
        ...
        IF (INFO /= 0) THEN
            INFO = -3
            SEALLOCATE(col_marker, work_values, work_ind)
            RETURN
        END IF
        ...
        SO i = 1, M
            ...
            ! Process row i of A
            IF (ALPHA /= 0.0_real32) THEN
                SO ia = A%row_ptr(i), A%row_ptr(i+1)-1
                    ja = A%col_ind(ia)
                    IF (col_marker(ja) < i) THEN
                        col_marker(ja) = i
                        nnz_row = nnz_row + 1
                        work_ind(nnz_row) = ja
                        work_values(ja) = 0.0_real32
                    END IF
            ...
            ! Process row i of B
            IF (BETA /= 0.0_real32) THEN
                SO ib = B%row_ptr(i), B%row_ptr(i+1)-1
                    jb = B%col_ind(ib)
                    IF (col_marker(jb) < i) THEN
                        col_marker(jb) = i
                        nnz_row = nnz_row + 1
                        work_ind(nnz_row) = jb
                        work_values(jb) = 0.0_real32
                    END IF
    ...
    SUBROUTINE DSPADD2_CSC(M, N, ALPHA, A, BETA, B, C, INFO)
        INTEGER(int32), INTENT(IN) :: M, N
        REAL(real32), INTENT(IN) :: ALPHA, BETA
        TYPE(sparse_csc_s), INTENT(IN) :: A, B
        TYPE(sparse_csc_s), INTENT(OUT) :: C
        INTEGER(int32), INTENT(OUT) :: INFO
        ...
        IF (INFO /= 0) THEN
            INFO = -3
            SEALLOCATE(row_marker, work_values, work_ind)
            RETURN
        END IF
        ...
        SO j = 1, N
            ...
            IF (ALPHA /= 0.0_real32) THEN
                SO ia = A%col_ptr(j), A%col_ptr(j+1)-1
                    ra = A%row_ind(ia)
                    IF (row_marker(ra) < j) THEN
                        row_marker(ra) = j
                        nnz_col = nnz_col + 1
                        work_ind(nnz_col) = ra
                        work_values(ra) = 0.0_real32
                    END IF
            ...
            IF (BETA /= 0.0_real32) THEN
                SO ib = B%col_ptr(j), B%col_ptr(j+1)-1
                    rb = B%row_ind(ib)
                    IF (row_marker(rb) < j) THEN
                        row_marker(rb) = j
                        nnz_col = nnz_col + 1
                        work_ind(nnz_col) = rb
                        work_values(rb) = 0.0_real32
                    END IF
