...
!>
!> \details
!> ZSPREAD reads a sparse matrix from a file in Matrix Market format.
...
SUBROUTINE ZSPREAD(FILENAME, FORMAT, SPARSE, INFO)
    ...
    TYPE(sparse_coo_z), INTENT(OUT) :: SPARSE
    ...
    ZO k = 1, nnz
        ...
        IF (ios /= 0) THEN
            INFO = 4
            ZEALLOCATE(row_ind, col_ind, values)
            CLOSE(unit)
            RETURN
        END IF
    ...
    CALL ZCOOALLOC(nrows, ncols, nnz_read, SPARSE, INFO)
    IF (INFO /= 0) THEN
        INFO = 3
        ZEALLOCATE(row_ind, col_ind, values)
        RETURN
    ...
    CALL ZCOOINIT(row_ind(1:nnz_read), col_ind(1:nnz_read), &
