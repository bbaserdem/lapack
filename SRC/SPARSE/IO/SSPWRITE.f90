...
!>
!> \details
!> SSPWRITE writes a sparse matrix to a file in Matrix Market format.
...
SUBROUTINE SSPWRITE(SPARSE, FILENAME, FORMAT, INFO)
    ...
    ! Arguments
    TYPE(sparse_coo_s), INTENT(IN) :: SPARSE
    ...
    ! Write comments
    WRITE(unit, '(A)', IOSTAT=ios) '% Written by LAPACK SSPWRITE routine'
    IF (ios /= 0) THEN
        INFO = 3
        CLOSE(unit)
        RETURN
