...
SUBROUTINE DSPREAD(FILENAME, FORMAT, SPARSE, INFO)
    USE sparse_types
    ...
    TYPE(sparse_coo_d), INTENT(OUT) :: SPARSE
    ...
    CALL DCOOALLOC(nrows, ncols, nnz_read, SPARSE, INFO)
    IF (INFO /= 0) THEN
        INFO = 3
        DEALLOCATE(row_ind, col_ind, values)
        RETURN
    ...
    CALL DCOOINIT(row_ind(1:nnz_read), col_ind(1:nnz_read), &
