!> \brief \b CCSCFREE deallocates CSC sparse matrix memory
!>
!> \par Purpose:
!> =============
!>
!> CCSCFREE deallocates all dynamically allocated memory associated
...
SUBROUTINE CCSCFREE(CSC, INFO)
    ...
    IF (ALLOCATED(CSC%col_ptr)) THEN
        CEALLOCATE(CSC%col_ptr, STAT=ierr)
        IF (ierr /= 0) THEN
    ...
    IF (ALLOCATED(CSC%row_ind)) THEN
        CEALLOCATE(CSC%row_ind, STAT=ierr)
        IF (ierr /= 0) THEN
            INFO = SPARSE_ERR_ALLOC
            RETURN
        END IF
    ...
    IF (ALLOCATED(CSC%values)) THEN
        CEALLOCATE(CSC%values, STAT=ierr)
        IF (ierr /= 0) THEN
            INFO = SPARSE_ERR_ALLOC
            RETURN
        END IF
