!> \brief \b SCSCMV performs matrix-vector multiplication using CSC format
!>
!> \par Purpose:
!> =============
!>
!> SCSCMV performs one of the matrix-vector operations
!>
...
SUBROUTINE SCSCMV(TRANS, M, N, ALPHA, CSC, X, INCX, BETA, Y, INCY)
        ...
        IF (INCY == 1) THEN
            SO j = 1, N
                IF (X(jx) /= 0.0_real32) THEN
                    temp = ALPHA * X(jx)
                    SO k = CSC%col_ptr(j), CSC%col_ptr(j+1) - 1
                        i = CSC%row_ind(k)
