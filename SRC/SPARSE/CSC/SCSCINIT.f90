!> \brief \b SCSCALLOC allocates a CSC sparse matrix
!>
!> \par Purpose:
!> =============
!>
!> SCSCALLOC allocates memory for a CSC format sparse matrix with specified
...
SUBROUTINE SCSCALLOC(NROWS, NCOLS, NNZ, CSC, INFO)
    ...
    TYPE(sparse_csc_s), INTENT(OUT) :: CSC
    ...
    IF (ierr /= 0) THEN
        ...
        IF (ALLOCATED(CSC%col_ptr)) SEALLOCATE(CSC%col_ptr)
        INFO = SPARSE_ERR_ALLOC
        CSC%nnz_alloc = 0
        RETURN
    END IF
...
END SUBROUTINE SCSCALLOC

!> \brief \b SCSCINIT initializes a CSC sparse matrix from arrays
!>
!> \par Purpose:
!> =============
!>
!> SCSCINIT initializes a CSC format sparse matrix from provided arrays
...
SUBROUTINE SCSCINIT(COLPTR, ROWIND, VAL, NCOLS, NNZ, CSC, INFO)
    USE sparse_types_extended
    USE sparse_constants
    IMPLICIT NONE
