!> \brief \b SDEN2COO converts dense matrix to COO format
!>
!> \par Purpose:
!> =============
!>
!> SDEN2COO converts a dense matrix to COO (Coordinate List) sparse format,
!> skipping zero elements according to the tolerance SPARSE_ZERO_TOL.
!>
!> \param[in] SENSE
!>          SENSE is SOUBLE PRECISION array, dimension (LDA,N)
!>          The input dense matrix.
!>
!> \param[in] LDA
!>          LDA is INTEGER
!>          The leading dimension of SENSE. LDA >= max(1,M).
!>
!> \param[in] M
!>          M is INTEGER
...
!>          On exit, the COO sparse matrix containing non-zero elements from SENSE.
...
SUBROUTINE SDEN2COO(SENSE, LDA, M, N, COO, INFO)
    ...
    ! Arguments
    REAL(real32), INTENT(IN) :: SENSE(LDA,*)
    ...
    SO j = 1, N
        SO i = 1, M
            val = SENSE(i,j)
            IF (ABS(val) > SPARSE_ZERO_TOL) THEN
