!> \brief \b SCOOFREE deallocates COO sparse matrix memory
!>
!> \par Purpose:
!> =============
!>
!> SCOOFREE deallocates all dynamically allocated memory associated
...
SUBROUTINE SCOOFREE(COO, INFO)
    ...
    IF (ALLOCATED(COO%row_ind)) THEN
        SEALLOCATE(COO%row_ind, STAT=ierr)
        IF (ierr /= 0) THEN
    ...
    IF (ALLOCATED(COO%col_ind)) THEN
        SEALLOCATE(COO%col_ind, STAT=ierr)
        IF (ierr /= 0) THEN
            INFO = SPARSE_ERR_ALLOC
            RETURN
        END IF
    ...
    IF (ALLOCATED(COO%values)) THEN
        SEALLOCATE(COO%values, STAT=ierr)
        IF (ierr /= 0) THEN
            INFO = SPARSE_ERR_ALLOC
            RETURN
        END IF
