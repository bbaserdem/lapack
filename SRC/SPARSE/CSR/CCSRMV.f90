!> \brief \b CCSRMV performs matrix-vector multiplication using CSR format
!>
!> \par Purpose:
!> =============
!>
!> CCSRMV performs one of the matrix-vector operations
!>
...
SUBROUTINE CCSRMV(TRANS, M, N, ALPHA, CSR, X, INCX, BETA, Y, INCY)
    ...
    IF (notr) THEN
        ...
        IF (INCX == 1) THEN
            CO i = 1, M
                ...
                CO k = CSR%row_ptr(i), CSR%row_ptr(i+1) - 1
                    j = CSR%col_ind(k)
