...
SUBROUTINE SCSRCSC(TRANSA, TRANSB, M, N, K, ALPHA, A, B, BETA, C)
    ...
    TYPE(sparse_csr_s), INTENT(IN) :: A
    TYPE(sparse_csc_s), INTENT(IN) :: B
    TYPE(sparse_csr_s), INTENT(INOUT) :: C
    ...
    REAL(real32), ALLOCATABLE :: work_values(:)
    ...
    IF (.NOT. nota .AND. .NOT. LSAME(TRANSA, 'T')) THEN
        PRINT *, 'SCSRCSC: Invalid TRANSA = ', TRANSA
        RETURN
    END IF
    
    IF (.NOT. notb .AND. .NOT. LSAME(TRANSB, 'T')) THEN
        PRINT *, 'SCSRCSC: Invalid TRANSB = ', TRANSB
        RETURN
    END IF
    ...
    IF (nota) THEN
        IF (A%nrows /= M .OR. A%ncols /= K) THEN
            PRINT *, 'SCSRCSC: Dimension mismatch in A'
            RETURN
        END IF
    ELSE
        IF (A%ncols /= M .OR. A%nrows /= K) THEN
            PRINT *, 'SCSRCSC: Dimension mismatch in transposed A'
            RETURN
        END IF
    END IF
    ...
    IF (notb) THEN
        IF (B%nrows /= K .OR. B%ncols /= N) THEN
            PRINT *, 'SCSRCSC: Dimension mismatch in B'
            RETURN
        END IF
    ELSE
        IF (B%ncols /= K .OR. B%nrows /= N) THEN
            PRINT *, 'SCSRCSC: Dimension mismatch in transposed B'
            RETURN
        END IF
    END IF
    ...
    IF (BETA == 0.0_real32) THEN
        ...
        C%ncols = N
        C%nnz_alloc = nnz_est
        IF (ALLOCATED(C%row_ptr)) SEALLOCATE(C%row_ptr)
        IF (ALLOCATED(C%col_ind)) SEALLOCATE(C%col_ind)
        IF (ALLOCATED(C%values)) SEALLOCATE(C%values)
        ALLOCATE(C%row_ptr(M+1), C%col_ind(nnz_est), C%values(nnz_est))
    ELSE
        ! We need to add to existing C - for now, error
        PRINT *, 'SCSRCSC: beta /= 0 not yet implemented'
        SEALLOCATE(col_marker, col_list, work_values)
        RETURN
    END IF
    ...
    SO i = 1, M
        ...
        ELSE
            ...
            PRINT *, 'SCSRCSC: Transposed A not yet implemented'
            SEALLOCATE(col_marker, col_list, work_values)
            RETURN
        END IF
...
CONTAINS
    ...
    RECURSIVE SUBROUTINE quicksort_int(arr, first, last)
        ...
        IF (first < last) THEN
            ...
            SO
                SO WHILE (arr(i) < pivot)
                    i = i + 1
                END SO
                SO WHILE (arr(j) > pivot)
                    j = j - 1
                END SO
                IF (i >= j) EXIT
                temp = arr(i)
    ...
    SUBROUTINE resize_csr(mat, new_size)
        TYPE(sparse_csr_s), INTENT(INOUT) :: mat
        INTEGER(int32), INTENT(IN) :: new_size
        INTEGER(int32), ALLOCATABLE :: temp_ind(:)
        REAL(real32), ALLOCATABLE :: temp_val(:)
        ...
        SEALLOCATE(mat%col_ind, mat%values)
