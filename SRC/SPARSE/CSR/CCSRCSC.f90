...
SUBROUTINE CCSRCSC(TRANSA, TRANSB, M, N, K, ALPHA, A, B, BETA, C)
    ...
    TYPE(sparse_csr_c), INTENT(IN) :: A
    TYPE(sparse_csc_c), INTENT(IN) :: B
    TYPE(sparse_csr_c), INTENT(INOUT) :: C
    ...
    IF (.NOT. nota .AND. .NOT. LSAME(TRANSA, 'T') .OR. LSAME(TRANSA, 'C')) THEN
        PRINT *, 'CCSRCSC: Invalid TRANSA = ', TRANSA
        RETURN
    END IF
    
    IF (.NOT. notb .AND. .NOT. LSAME(TRANSB, 'T') .OR. LSAME(TRANSA, 'C')) THEN
        PRINT *, 'CCSRCSC: Invalid TRANSB = ', TRANSB
        RETURN
    END IF
    ...
    IF (nota) THEN
        IF (A%nrows /= M .OR. A%ncols /= K) THEN
            PRINT *, 'CCSRCSC: Dimension mismatch in A'
            RETURN
        END IF
    ELSE
        IF (A%ncols /= M .OR. A%nrows /= K) THEN
            PRINT *, 'CCSRCSC: Dimension mismatch in transposed A'
            RETURN
        END IF
    END IF
    ...
    IF (notb) THEN
        IF (B%nrows /= K .OR. B%ncols /= N) THEN
            PRINT *, 'CCSRCSC: Dimension mismatch in B'
            RETURN
        END IF
    ELSE
        IF (B%ncols /= K .OR. B%nrows /= N) THEN
            PRINT *, 'CCSRCSC: Dimension mismatch in transposed B'
            RETURN
        END IF
    END IF
    ...
    IF (BETA == (0.0_real32, 0.0_real32)) THEN
        ...
        C%ncols = N
        C%nnz_alloc = nnz_est
        IF (ALLOCATED(C%row_ptr)) CEALLOCATE(C%row_ptr)
        IF (ALLOCATED(C%col_ind)) CEALLOCATE(C%col_ind)
        IF (ALLOCATED(C%values)) CEALLOCATE(C%values)
        ALLOCATE(C%row_ptr(M+1), C%col_ind(nnz_est), C%values(nnz_est))
    ELSE
        ! We need to add to existing C - for now, error
        PRINT *, 'CCSRCSC: beta /= 0 not yet implemented'
        CEALLOCATE(col_marker, col_list, work_values)
        RETURN
    END IF
    ...
    CO i = 1, M
        nnz_row = 0
        work_values = (0.0_real32, 0.0_real32)
        ...
        ELSE
            ...
            PRINT *, 'CCSRCSC: Transposed A not yet implemented'
            CEALLOCATE(col_marker, col_list, work_values)
            RETURN
        END IF
...
CONTAINS
    ...
    SUBROUTINE resize_csr(mat, new_size)
        TYPE(sparse_csr_c), INTENT(INOUT) :: mat
        ...
        CEALLOCATE(mat%col_ind, mat%values)
