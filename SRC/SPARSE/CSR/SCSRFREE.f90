!> \brief \b SCSRFREE deallocates CSR sparse matrix memory
!>
!> \par Purpose:
!> =============
!>
!> SCSRFREE deallocates all dynamically allocated memory associated
...
SUBROUTINE SCSRFREE(CSR, INFO)
    ...
    IF (ALLOCATED(CSR%row_ptr)) THEN
        SEALLOCATE(CSR%row_ptr, STAT=ierr)
        IF (ierr /= 0) THEN
    ...
    IF (ALLOCATED(CSR%col_ind)) THEN
        SEALLOCATE(CSR%col_ind, STAT=ierr)
        IF (ierr /= 0) THEN
            INFO = SPARSE_ERR_ALLOC
            RETURN
        END IF
    ...
    IF (ALLOCATED(CSR%values)) THEN
        SEALLOCATE(CSR%values, STAT=ierr)
        IF (ierr /= 0) THEN
            INFO = SPARSE_ERR_ALLOC
            RETURN
        END IF
