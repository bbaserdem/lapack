!> \brief \b SCSRALLOC allocates a CSR sparse matrix
!>
!> \par Purpose:
!> =============
!>
!> SCSRALLOC allocates memory for a CSR format sparse matrix with specified
...
SUBROUTINE SCSRALLOC(NROWS, NCOLS, NNZ, CSR, INFO)
    ...
    TYPE(sparse_csr_s), INTENT(OUT) :: CSR
    ...
    IF (ierr /= 0) THEN
        ...
        IF (ALLOCATED(CSR%row_ptr)) SEALLOCATE(CSR%row_ptr)
        INFO = SPARSE_ERR_ALLOC
        CSR%nnz_alloc = 0
        RETURN
    END IF
...
END SUBROUTINE SCSRALLOC

!> \brief \b SCSRINIT initializes a CSR sparse matrix from arrays
!>
!> \par Purpose:
!> =============
!>
!> SCSRINIT initializes a CSR format sparse matrix from provided arrays
...
!>
!> \param[in] VAL
!>          VAL is SOUBLE PRECISION array, dimension (NNZ)
!>          The non-zero values.
!>
!> \param[in] NROWS
!>          NROWS is INTEGER
...
SUBROUTINE SCSRINIT(ROWPTR, COLIND, VAL, NROWS, NNZ, CSR, INFO)
    USE sparse_types_extended
    USE sparse_constants
    IMPLICIT NONE
